...
uses
  ...
  Vcl.Mask, Vcl.ExtCtrls, Vcl.Imaging.pngimage, Vcl.ComCtrls, Winapi.IpHlpApi, Winapi.IpTypes,ShellAPI;
...
function TForm1.GetNetworkInfo: string;
var
  FSWbemLocator: OLEVariant;
  FWMIService: OLEVariant;
  FWbemObjectSet: OLEVariant;
  ...
  iValue: LongWord;
  Speed: Int64;
  SpeedFormatted,  NetConnectionID: string;
const
...
begin
  ...
  try
    ...
    FWbemObjectSet := FWMIService.ExecQuery(
      'SELECT Name, NetConnectionID, Speed FROM Win32_NetworkAdapter WHERE NetConnectionID IS NOT NULL',
      'WQL',
      0,
      null
    );
     Result := 'Interface de Rede'+ sLineBreak;
    oEnum := IUnknown(FWbemObjectSet._NewEnum) as IEnumVariant;
    while oEnum.Next(1, FWbemObject, iValue) = 0 do
    begin
      
     NetConnectionID := Trim(VarToStr(FWbemObject.Properties_.Item('NetConnectionID').Value));
     
    if (NetConnectionID = 'Ethernet') or (NetConnectionID = 'Wi-Fi') then
    begin
        Result := Result +('Name: ' + VarToStr(FWbemObject.Properties_.Item('Name').Value))+ sLineBreak;
      Result := Result +('NetConnectionID: ' + VarToStr(FWbemObject.Properties_.Item('NetConnectionID').Value))+ sLineBreak;

      Speed := FWbemObject.Properties_.Item('Speed').Value;
      if Speed = MAX_INT64 then
        SpeedFormatted := 'Informações de velocidade não disponíveis'
      else if Speed < 1000000000 then
        SpeedFormatted := FormatFloat('0.##', Speed / 1000000) + ' Mbps'
      else
        SpeedFormatted := FormatFloat('0.##', Speed / 1000000000) + ' Gbps';
      Result := Result +('Speed: ' + SpeedFormatted) + sLineBreak+sLineBreak;
...
Memo1.Lines.Add(GetNetworkInfo);
